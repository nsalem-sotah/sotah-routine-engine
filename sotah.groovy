/**
 *  SOTAH - Community's own Rule Engine
 *
 *  Based on the CoRE engine software created by Adrian Caramaliu <ady624("at" sign goes here)gmail.com>
 *
...
definition(
	name: "SOTAH${parent ? " - Smart Routine" : ""}",
	namespace: "SOTAH",
	author: "SOTAH",
	description: "SOTAH - Smart Routines",
	singleInstance: true,
...
def pageMain() {
	def res = dev()
	parent ? pageMainSOTAHPiston() : pageMainSOTAH()
}
...
/*** SOTAH PAGES																***/
/******************************************************************************/
private pageMainSOTAH() {
	initializeSOTAHStore()
    rebuildTaps()
	//SOTAH main page
	dynamicPage(name: "pageMain", title: "", install: true, uninstall: false) {
		...
		section(title:"") {
			href "pageGeneralSettings", title: "Settings", image: "https://cdn.rawgit.com/ady624/SOTAH/master/resources/images/icons/settings.png", required: false
		}
...
private pageInitializeDashboard() {
	//SOTAH Dashboard initialization
	def success = initializeSOTAHEndpoint()
	dynamicPage(name: "pageInitializeDashboard", title: "") {
		section() {
			if (success) {
				paragraph "Success! Your SOTAH dashboard is now enabled. Tap Done to continue", required: false
			} else {
				paragraph "Please go to your SmartThings IDE, select the My SmartApps section, click the 'Edit Properties' button of the SOTAH app, open the OAuth section and click the 'Enable OAuth in Smart App' button. Click the Update button to finish.\n\nOnce finished, tap Done and try again.", title: "Please enable OAuth for SOTAH", required: true, state: null
			}
		}
	}
}
...
def pageGeneralSettings(params) {
	dynamicPage(name: "pageGeneralSettings", title: "General Settings", install: false, uninstall: false) {
		section("About") {
			paragraph app.version(), title: "SOTAH Version", required: false
		...
		section("Remove SOTAH") {
			href "pageRemove", title: "", description: "Remove SOTAH", required: false
		}
...
def pageDashboardTaps() {
	...
	dynamicPage(name: "pageDashboardTaps", title: "Dashboard Taps", install: false, uninstall: false) {
    	def taps = state.taps
        section("") {
        	href "pageDashboardTap", title: "Add a new tap", required: false, params: [id: 0], image: "https://cdn.rawgit.com/ady624/SOTAH/master/resources/images/tap.png"
        }
        if (taps.size()) {
            section("Taps") {
                for (tap in taps) {
                    href "pageDashboardTap", title: tap.n, description: "Runs ${buildNameList(tap.p, "and")}", required: false, params: [id: tap.i], image: "https://cdn.rawgit.com/ady624/SOTAH/master/resources/images/tap.png"
                }
            }
        }
    }
...
def pageIntegrateIFTTT() {
	return dynamicPage(name: "pageIntegrateIFTTT", title: "IFTTT™ Integration", nextPage: settings.iftttEnabled ? "pageIntegrateIFTTTConfirm" : null) {
		section() {
			paragraph "SOTAH can optionally integrate with IFTTT™ (IF This Then That) via the Maker channel, triggering immediate events to IFTTT™. To enable IFTTT™, please login to your IFTTT™ account and connect the Maker channel. You will be provided with a key that needs to be entered below", required: false
...
def pageIntegrateIFTTTConfirm() {
	if (testIFTTT()) {
		return dynamicPage(name: "pageIntegrateIFTTTConfirm", title: "IFTTT Integration", nextPage:"pageGeneralSettings") {
			section(){
				paragraph "Congratulations! You have successfully connected SOTAH to IFTTT."
			}
		}
	} else {
...
def pageResetSecurityToken() {
	return dynamicPage(name: "pageResetSecurityToken", title: "SOTAH Security Token") {
		section() {
			paragraph "CAUTION: Resetting the security token is an ireversible action. Once done, any integrations that rely on the security token, such as the SOTAH Dashboard, the IFTTT Maker channel used as an action, etc. will STOP working and will require your attention. You will need to update the security token everywhere you are currently using it.", required: true
...
def pageResetSecurityTokenConfirm() {
	state.endpoint = null
	initializeSOTAHEndpoint()
	return dynamicPage(name: "pageResetSecurityTokenConfirm", title: "SOTAH Security Token") {
		section() {
...
private pageMainSOTAHPiston() {
	//SOTAH Piston main page
	//dev()
	state.run = "config"
	configApp()
...
def initialize() {
	parent ? initializeSOTAHPiston() : initializeSOTAH()
}
...
def publishVariables() {
	...
	for (variable in globalVars) {
        ...
        if (oldValue != newValue) {
            sendLocationEvent(name: "variable", value: name, displayed: true, linkText: "SOTAH Global Variable", isStateChange: true, descriptionText: "Variable $name changed from '$oldValue' to '$newValue'", data: [app: "SOTAH", oldValue: oldValue, value: newValue])
        }
	}
    state.globalVars = [:]
}
...
/******************************************************************************/
/*** SOTAH INITIALIZATION METHODS											***/
/******************************************************************************/

def initializeSOTAH() {
	initializeSOTAHStore()
	refreshPistons()
	subscribe(location, "SOTAH", coreHandler)
    ...
    for(app in apps) {
    	if ((recoverAll || (recovery[app.id] && (recovery[app.id] < threshold))) && (!excludeAppId || (excludeAppId != app.id))) {
        	...
        	if (recoverAll || excludeAppId) {
				sendLocationEvent(name: "SOTAH Recovery [${app.id}]", value: "", displayed: true, linkText: "SOTAH/${app.label} Recovery", isStateChange: true)
            } else {
            	def message = "Found SOTAH Piston '${app.label ?: app.name}' about ${Math.round((now() - recovery[app.id])/ 1000)} seconds past due, attempting recovery"
...
private initializeSOTAHEndpoint() {
	if (!state.endpoint) {
		try {
...
def api_ifttt() {
	...
	if (eventName) {
		sendLocationEvent([name: "ifttt", value: eventName, isStateChange: true, linkText: "IFTTT event", descriptionText: "SOTAH has received an IFTTT event: $eventName", data: data])    	
    }
...
/******************************************************************************/
/*** SOTAH PISTON INITIALIZATION METHODS										***/
/******************************************************************************/

def initializeSOTAHPiston() {
...
def initializeSOTAHPistonStore() {
	state.temp = state.temp ?: [:]
	state.cache = [:]
...
private subscribeToAll(appData) {
    ...
    subscribe(location, "SOTAH Recovery [${app.id}]", recoveryHandler)
...
/******************************************************************************/
/*** SOTAH PISTON CONFIGURATION METHODS										***/
/******************************************************************************/
...
/******************************************************************************/
/*** SOTAH PISTON PUBLISHED METHODS											***/
/******************************************************************************/
